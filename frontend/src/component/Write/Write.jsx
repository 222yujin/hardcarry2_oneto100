...
function WriteForm(props) {
  ...
  const openModal = async () => {
    console.log(values);
    if(values.choice==true){//이름 연락처 미 입력시
      if((values.name==null||values.phone==null)){
        alert("연락 받으실 정보를 정확히 입력해주세요!")
        return;
      }
      if(((values.name!=null&&values.phone!=null)&&values.private==false)){
     ...
     await fetch(
        "http://3.35.152.195/api/diary/createDiary",
        {
          method: "POST",
          headers: {
                ...
                <div className={styles.submitdiaryscope}>
                  ...
                  <div className={styles.write_private}>
                    <input
                      ...
                      checked={checkedButtons.includes('private') ? true : false}
                      placeholder="(선택) 연락처 적고 경품 받기"
                    />
                    개인정보 활용동의
                  </div>
